...
begin
	...
	V .+= 0.06 .* rand(n, n) #add rando bits of culture to cells
	for i in 1:8000
	    step!(U, V)
	end
...
#=	extra implementationulary tippies:
	...
	-yea you should make sure you allocate and make a cuppa while the final visualization runs
		...
		-yes I know I should have done it this way from the start
...
the logic of scaling is simple:

ratio = n_new/n_old

sooo this should work but...it might not.

# 1. Define your original 300x300 node positions and radius
old_nodes = [(50,60), (120,80), (90,140), (140,135)]
old_radius = 6

# 2. Calculate your spatial ratio dynamically
spatial_ratio = n / 300
new_radius = round(Int, old_radius * spatial_ratio)

# 3. Scale and execute the injection loop
for (old_x, old_y) in old_nodes
    # Multiply coordinate by ratio and convert to an Integer index
    x = round(Int, old_x * spatial_ratio)
    y = round(Int, old_y * spatial_ratio)
    
    # Apply the perfectly scaled slice bounds
    U[x - new_radius : x + new_radius, y - new_radius : y + new_radius] .= 0.50
...
#= 	Extensive comments as to how this works bc who am I, Euler? 
		PDEs? more like Pleese oh geez, amirite?
...
Params: 
	...
	F = feed rate, how much U returns
	k = kill rate, how fast V dies 
	
	UVV = U .* V .* V is reaction expression, dots are element by element multiplication

	instead of matrix algebra for every darn cell julia is doing:
	reaction = U * V * V

The culture (V) blooms anywhere there is substrate/food (U) AND already a critical mass of culture (V)


	update food:
	U .+= Du .* lap(U) .- UVV .+ F .* (1 .- U) # Food gets eaten, and slowly replenished by Feed rate (F)
	
		Du .* lap(U) [food diffuses]
	
		.- UVV [reaction consumes food]
	...
	plus food being fed back in

	Update culture:
	V .+= Dv .* lap(V) .+ UVV .- (F + k) .* V # culture explodes into existence, and gets drained by the kill rate (k)

		xDv .* lap(V) [culture diffuses/spreads.]

		.+ UVV [reaction makes more culture.]
...
*This category is not really Gray–Scott stuff, and belongs more to oscillatory/excitable systems like the Belousov–Zhabotinsky reaction. (the lilac/brominey red oscilatory one)*
*Gray–Scott can sometimes approach these behaviors, but usually not textbook canonical forms.*

**spirals** — rotating pinwheels, rare, lovely, and parameter-sensitive. More characteristic of oscillatory reaction-diffusion systems like the Belousov–Zhabotinsky reaction (the red-and-lilac bromine-y oscillator in a dish). Think about what happens when a wave front curves back on itself.

> also: target waves — concentric rings expanding outward like a pebble in a pond  
> traveling bands, ring waves
...
**quasi-solitons** — stable spots that travel across the grid without changing shape or dying. very specific parameter regime, possibly magic. think: what would it take for V to be stable enough to hold its shape but restless enough to move? Not sure if possible in this sys. Might be really hard to find.
...
**edge turbulence** — the bulk of the field has its shit together. the boundary does not. where two stable regions meet and refuse to play nice, the system sits on the precipice between two attractors and just churns and gurns there forever.
...
manifest_format = "2.0"
project_hash = "d8da3f8c3555f89c013345c778de9b9b517c415b"
